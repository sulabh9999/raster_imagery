...
def sliding_window(frame, window_size, stride=None):
	...
	if not window_size or len(window_size) != 2:
	  raise ValueError(f'Incorrect window size {window_size}, expected (h, w)')
...
def get_image_chunks(tif, window_size, stride=None, img_count=None):
	...
	for frame in sliding_window(frame=(res_height, res_width), window_size=window_size, stride=stride):
